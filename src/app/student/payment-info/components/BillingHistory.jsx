...
const BillingHistoryData = ({
  ...
  return <tr>
      ...
      <td>
        {paymentMethod.number ? <img src={paymentMethod.image} height={40} width={40} className="h-40px" alt="image" /> : <img src={paymentMethod.image} height={21} width={80} className="w-80px" alt="image" />}
        <span className="ms-2">{paymentMethod.number}</span>
      </td>
      <td>
        <span className={`badge ${status === 'paid' ? 'bg-success text-success' : status === 'pending' ? 'bg-orange text-orange' : 'bg-danger text-danger'}  bg-opacity-10 `}>
          {status}
        </span>
      </td>
      <td>
