      ...
      <div className="position-relative">
        ...
        <div className="card-img-overlay d-flex align-items-start flex-column p-3">
          <div className="w-100 mt-auto">
            {hasLive ? <Link to="" className="badge text-success bg-white fs-6 rounded-1">
                <FaCircle className="text-success me-2" />
                Live
              </Link> : <Link to="" className="badge text-bg-white fs-6 rounded-1">
      ...
      <Container>
        ...
        <Row>
          <div className="tiny-slider arrow-round arrow-creative arrow-blur arrow-hover">
            {allEvents && <TinySlider settings={eventSliderSettings}>{allEvents?.map((event, idx) => <EventCard event={event} key={idx} />)}</TinySlider>}
          </div>
        </Row>
      </Container>
    </section>;
