      ...
      <TabContainer defaultActiveKey={1}>
        <Nav className=" nav-pills nav-pills-bg-soft px-3" id="book-pills-tab" role="tablist">
          <NavItem className="me-2 me-sm-4" role="presentation">
            <NavLink eventKey={1} className="mb-0 " id="book-pills-tab-1" data-bs-toggle="pill" data-bs-target="#book-pills-1" type="button" role="tab" aria-controls="book-pills-1" aria-selected="true">
              Author
            </NavLink>
          </NavItem>
          <NavItem className="me-2 me-sm-4" role="presentation">
            <NavLink eventKey={2} className="mb-0" id="book-pills-tab-2" data-bs-toggle="pill" data-bs-target="#book-pills-2" type="button" role="tab" aria-controls="book-pills-2" aria-selected="false">
              Reviews
            </NavLink>
          </NavItem>
          <NavItem className="me-2 me-sm-4" role="presentation">
            <NavLink eventKey={3} className="mb-0" id="book-pills-tab-3" data-bs-toggle="pill" data-bs-target="#book-pills-3" type="button" role="tab" aria-controls="book-pills-3" aria-selected="false">
              Shipping Method
            </NavLink>
          </NavItem>
        </Nav>
              ...
              <Col md={8}>
                <Row className=" align-items-center text-center">
                  {progress.map((item, idx) => <Fragment key={idx}>
                      ...
                      <Col xs={6} sm={4}>
                        <ul className="list-inline mb-0">
                          {Array(Math.floor(progress.length - idx)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1 small">
                                <FaStar size={14} className="text-warning" />
                              </li>)}
                          {!Number.isInteger(progress.length - idx) && <li className="list-inline-item me-1 small">

                              <FaStarHalfAlt size={14} className="text-warning" />
                            </li>}
                          {progress.length - idx < 5 && Array(5 - Math.ceil(progress.length - idx)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1 small">
                                  <FaRegStar size={14} className="text-warning" />
                                </li>)}
                        </ul>
                      </Col>
