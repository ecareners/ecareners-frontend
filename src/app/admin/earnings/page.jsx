...
const InvoiceHistoryCard = ({
  ...
  return <tr>
      ...
      <td>
        <img src={paymentMethod.image} className={` ${paymentMethod.type === 'paypal' ? 'h-20px' : 'h-50px'}`} height={paymentMethod.type === 'paypal' ? 21 : 50} alt="paymentMethodImg" />
      </td>
      <td>
        ${amount}&nbsp;
      ...
      <td>
        <div className={`badge bg-${status === 'paid' ? 'success' : status === 'pending' ? 'orange' : 'danger'} bg-opacity-10 text-${status === 'paid' ? 'success' : status === 'pending' ? 'orange' : 'danger'}`}>
          {status}
        </div>
      </td>
      <td>
