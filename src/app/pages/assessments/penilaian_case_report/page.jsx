...
const PenilaianCaseReportPage = () => {
  ...
  const [assessmentForm, setAssessmentForm] = useState({
    aspek_casport_1: '', aspek_casport_2: '', aspek_casport_3: '', aspek_casport_4: '',
    comments: ''
  });
  ...
  const fetchExistingAssessment = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/users/${studentId}/assessments/penilaian_case_report`);
      if (response.ok) {
        const data = await response.json();
        if (data.length > 0) {
  ...
  const calculateFinalScore = () => {
    const scores = [];
    for (let i = 1; i <= 4; i++) {
      scores.push(parseInt(assessmentForm[`aspek_casport_${i}`]) || 0);
    }
    
    const total = scores.reduce((sum, score) => sum + score, 0);
    return (total / 16) * 100; // Formula: total aspek/16 x 100
  };
  ...
  const handleAssessmentSubmit = async (e) => {
    ...
    const requestData = {
      ...
      assessment_type: 'penilaian_case_report',
    ...
    console.log('📤 Sending case report assessment data:', requestData);
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('✅ Case report assessment saved successfully:', responseData);
        // Refresh assessment data
        await fetchExistingAssessment();
        setSaving(false);
      } else {
        const errorData = await response.json();
        console.error('❌ Case report assessment save failed:', errorData);
          ...
          <div className="d-flex justify-content-between align-items-center mb-4">
            <div>
              <h2 className="mb-1">
                ...
                Penilaian Case Report Assessment
              </h2>
              <p className="text-muted mb-0">Assess student's case report quality</p>
            </div>
            <Button 
                ...
                <div className="row">
                  {Array.from({length: 4}, (_, i) => i + 1).map((index) => (
                    <div key={index} className="col-md-6 mb-3">
                      <Form.Group>
                        <Form.Label>Aspect {index} (1-4 points)</Form.Label>
                        <Form.Control
                          type="number"
                          min="1"
                          max="4"
                                                  value={assessmentForm[`aspek_casport_${index}`]}
                        onChange={(e) => setAssessmentForm({...assessmentForm, [`aspek_casport_${index}`]: e.target.value})}
                          required
              ...
              <Card.Body>
                <div className="row">
                  <div className="col-md-6">
                    ...
                    <p><strong>Assessor:</strong> {assessment.assessor_name || 'N/A'}</p>
                    <p><strong>Comments:</strong> {assessment.comments || 'No comments'}</p>
                  </div>
                  <div className="col-md-6">
                    <p><strong>Final Score:</strong> 
                      <Badge bg={getScoreColor(assessment.score || 0)} className="ms-2">
                        {assessment.score || 0}%
                      </Badge>
                    </p>
                  </div>
                    ...
                    <h6>Detailed Scoring:</h6>
                       ...
                       {Array.from({length: 4}, (_, i) => i + 1).map((index) => (
                         <div key={index} className="col-md-3 mb-2">
                           <p><strong>Aspect {index}:</strong> {detailedData[`aspek_casport_${index}`]}/4</p>
                         </div>
                       ))}
                     </div>
                  </div>
...
export default PenilaianCaseReportPage; 