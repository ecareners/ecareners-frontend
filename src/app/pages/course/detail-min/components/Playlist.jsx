import { Accordion, AccordionBody, AccordionHeader, AccordionItem, Button, Col, Modal, ModalBody, ModalFooter, ModalHeader, ProgressBar, Row } from 'react-bootstrap';
...
const Playlist = () => {
  ...
  const playListData = useFetchData(getAllPlaylist);
  return <>
      <Accordion defaultActiveKey="0" className="accordion-icon accordion-bg-light" id="accordionExample2">
        {playListData?.map((playlist, idx) => <AccordionItem eventKey={`${idx}`} className={clsx(playListData.length - 1 != idx ? 'mb-3' : 'mb-0')} key={idx}>
            ...
            <AccordionBody className="mt-3">
              <div className="vstack gap-3">
                <div className="overflow-hidden">
                  ...
                  <ProgressBar variant="primary" now={idx == 0 ? 30 : 0} className="progress-sm bg-opacity-10 aos" data-aos="slide-right" data-aos-delay={200} data-aos-duration={1000} data-aos-easing="ease-in-out" />
                </div>
                {playlist.lectures.map((lecture, idx) => <Fragment key={idx}>
                    {lecture.playing ? <div className="p-2 bg-success bg-opacity-10 rounded-3">
                        ...
                        <div className="position-relative d-flex align-items-center">
                          {lecture.isPremium ? <Button variant="light" size="sm" className="btn btn-round mb-0 stretched-link position-static" onClick={toggle}>
                              <BsLockFill size={10.5} />
                            </Button> : <Button variant="danger-soft" size="sm" className="btn btn-round mb-0 stretched-link position-static">
                              <FaPlay className="me-0" size={11} />
                            </Button>}
                          <span className="d-inline-block text-truncate ms-2 mb-0 h6 fw-light w-200px">{lecture.title}</span>
                        </div>
